...
function run_program() {
    export procs=$1
    LOGFILE="$FINAL/out/mpi_strong_${procs}procs.log"
    
    # Run the program and save full output to log file
    # Extract time from output
    mpirun -np $procs $EXE $args -c 1 2>&1 | tee "$LOGFILE" | grep "tock" | cut -d' ' -f3 | tail -n1
    
    # Move output image to out directory if it exists
    if [ -f "$FINAL/img.jpg" ]; then
        mv "$FINAL/img.jpg" "$FINAL/out/mpi_strong_${procs}procs.jpg" 2>/dev/null || true
    fi
}
